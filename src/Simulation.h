...
class Simulation
{
 ...
 private:
	...
	QuadPrimitive plane{ glm::vec2(10.0f,10.0f)};
	std::vector<PlaneCollider> m_planeColliders;
	BoxCollider boxCollider{glm::vec3{-5,-5,-5}, glm::vec3{5,5,5}};
	std::array<Transform,6> m_planeTransforms;

	SpherePrimitive sphere{0.1f,24,16};
	Shader shader, m_instancedShader;

	const size_t SPHERE_COUNT{25000};
	SPHSolverCPU m_solver{ 1.0/60.0, SPHERE_COUNT, m_planeColliders,boxCollider};
